...
The `release` job needs `df-cli` — the DataFlex package manager — to validate, pack, and push the package to the DataFlex package repository at `packages.dataflex.dev`.
`df-cli` ships with a licensed DataFlex installation, which is not available on GitHub's hosted runners.
The job therefore runs on a self-hosted Windows runner (labels: `self-hosted`, `windows`, `dataflex`).
...
2. Create a **WebAPIs** package in that repository (this is the package name for `Kruse-Net/WebAPIs`).

There is currently no way to create an access token.

## Security notes (read this first)

GitHub's documentation warns that **self-hosted runners should almost never be used for public repositories**,
because anyone can open a pull request against the repo and potentially execute code on your runner.
This workflow is designed to limit that risk:

- The `release` job (the only job that runs on the self-hosted runner) triggers **only on pushes of version tags (`v*`)** — never on `pull_request` events,
  so untrusted PR content can never run jobs on this machine.
- The `build` job runs on GitHub's hosted runners, so pull requests never touch the self-hosted runner either.
- Only official GitHub Actions (`actions/checkout@v7`, `actions/setup-node@v7`, `actions/upload-artifact@v7`) are used.

Additional recommendations:

- Keep the machine's exposure minimal: it only needs outbound HTTPS (port 443) to GitHub and to `packages.dataflex.dev`.
- Anyone with write access to this repository can push a tag and therefore run code on the runner.
  Only grant repo write access to people you trust with that.
...
| DataFlex 26 installed **machine-wide**         | The release job invokes `df-cli`, which is part of the DataFlex installation (`C:\Program Files\DataFlex 26.0\Bin\df-cli.exe`). It must be on the **system `PATH`**. Verify with `df-cli system` in an elevated shell. |
| Valid DataFlex license                         | `df-cli build` requires a licensed toolchain.                                                                                                                                                                          |
| PowerShell Core                                | Version 7.x                                                                                                                                                                                                            |
...
| Account that is owner/admin of this repository | Needed to create the runner                                                                                                                                                                                            |

Node.js does **not** need to be installed system-wide: the job installs Node.js 24 via `actions/setup-node` on every run.

## Step 1: Create a user account for the runner

1. Create a dedicated Windows user account for the runner (e.g., `svc-github-runner`).
2. Open a command prompt or PowerShell as that user account and run `df-cli login` to sign in to `packages.dataflex.dev` and store the credentials in the user profile.
...
When prompted for the service account, enter the username of the dedicated account you created (e.g., `svc-github-runner`).
...
1. Update `package.version` in `WebAPIs.sws` and tag and push the commit with a throwaway version, e.g. '9.9.9':
...
2. Watch the workflow run. Success means: `npm` build → `df-cli system` → `df-cli config`/`df-cli build` on both workspaces → `df-cli package push` → `WebAPIs-9.9.9.zip` and `dist/WebAPIsDemo.zip` uploaded as artifacts → GitHub Release `v9.9.9`.
...
For a real release, push `v<version>` with the `package.version` in `WebAPIs.sws` matching the tag.
...
`df-cli` is not updated independently — it is part of the DataFlex installation.
Update DataFlex the usual way (machine-wide), then restart the runner service so the service account picks up the new `PATH`/installation.
